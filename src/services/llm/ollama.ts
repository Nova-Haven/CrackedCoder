...
export class OllamaService implements LLMService {
  ...
  async initialize(config: LLMConfig): Promise<void> {
    try {
      ...
      console.log(
        `Testing connection to Ollama at: ${this.endpoint}/api/version`
      );
      ...
      throw new LLMServiceError(
        "Failed to connect to Ollama API. Make sure Ollama is running and accessible at " +
          this.endpoint
      );
    }
  }
  ...
  async processScreenshots(
    ...
    try {
      ...
      console.log(
        `Sending request to Ollama at: ${this.endpoint}/api/generate`
      );
      ...
      try {
        ...
        throw new LLMServiceError("Failed to parse Ollama response");
      }
    } catch (error) {
      console.error("Error processing screenshots with Ollama:", error);
      throw new LLMServiceError(
        `Ollama processing error: ${(error as Error).message}`
      );
    }
  }
}
